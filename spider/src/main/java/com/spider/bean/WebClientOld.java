...
public class WebClientOld {
    ...
    private Logger logger = LoggerFactory.getLogger(WebClientOld.class);

    public WebClientOld() {
    }

    public WebClientOld build(Long timeout) {
        this.timeout = timeout;
        return this;
    }

    public WebClientOld build(Boolean needProxy) {
        this.needProxy = needProxy;
        return this;
    }

    public WebClientOld build(String proxyHost, int port) {
        this.proxyHost = new ProxyHost(proxyHost, port);
        return this;
    }

    public WebClientOld init() {
    ...
    private void buildHostParams() {
        if (hostParams == null) {
            ...
            //3.1
            client.getHttpConnectionManager().getParams().setParameter(HttpConnectionManagerParams.CONNECTION_TIMEOUT, 3000L);
            client.getHttpConnectionManager().getParams().setParameter(HttpConnectionManagerParams.SO_TIMEOUT, 10000L);
...
//        ((PostMethod) method).setRequestBody();
        ...
        } else if (ParamType.MULTIPART.equals(request.getParamType())) {
...
//        response.setEntity(processStream(method.getResponseBodyAsStream(), method.getResponseHeader("Content-Encoding")));
